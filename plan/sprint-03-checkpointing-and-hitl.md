...
- **Checkpoint deserialization is a gadget surface (S01-13 security review):** MAF's checkpointing resolves types polymorphically from wire data (`Checkpointing.TypeId(AssemblyName, TypeName)`, `JsonWireSerializedValue`, `IDelayedDeserialization`). Once checkpoint rows come from SQL Server, they are deserialized input into the worker process: constrain `TypeId` resolution to an allowlist of CBIX message types and integrity-protect rows, or a tamperable checkpoint is remote code influence. Also: when OTel workflow telemetry lands (S03-09/10), `WorkflowTelemetryOptions.EnableSensitiveData` stays false — the payloads it would capture are document paths and raw model output.
