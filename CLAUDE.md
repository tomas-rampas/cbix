...
Composition root, as shipped: `Cbix.Core.Hosting.AddCbixWorkflow` composes the workflow graph, ingest stack, and the two local profiles — provider-free, which is what lets tests and S01-09's stub-client agnosticism run exercise the real composition without the executable. The host's `Cbix.Worker.CbixWorkerHostExtensions.AddCbixWorker` layers on everything that names a provider: the Anthropic adapter and its credential (via the secret resolver), the keyed document-bound agent factories, the Claude document-content profile source, and configuration binding — because Core's containment tests forbid it naming an adapter, and which deployment uses which provider is a host decision by design. The active profile is selected purely by configuration (`Cbix:Provider:DocumentPresentation`); profiles are run-scoped; the production container runs with `ValidateScopes`/`ValidateOnBuild`. `src/Cbix.Worker` stays a thin `Program.cs` that calls the extension and runs.
...
- The generic-vision profile rasterises untrusted PDFs through **PDFium** (native, Chromium-derived, via PDFtoImage). PDFium memory-safety CVEs arrive as Chrome patches and are **invisible to the NuGet audit gate** — tracking them against the pinned build is a named operational obligation (design §11 addendum); a native fault kills the worker process, with checkpointing bounding the data loss.
- `DocumentReference` accepts `file://` locations only; widening to object-store schemes is a recorded design change, not a bug fix (design §11 addendum).
- Deployment constraint: the configured ingest root MUST be write-restricted to the ingest service principal; ingest-refusal security events (EventIds 1010–1019 family, registry in `Cbix.Core.Diagnostics.CbixEventIds`) are the instrument monitoring that assumption and must route to SIEM when logging providers land (Sprint 03).
- Checkpoint rows will carry the PDFPig text layer (client content in production) and MAF's checkpointing deserialises types from wire data — encryption-at-rest, retention, an allowlisted `TypeId` set, and text-layer/token integrity coupling are binding Sprint 03 obligations (see `plan/sprint-03-*.md`'s security block).
